...
async function runTest() {
  ...
  console.log("Running Okta addUserToGroup test...");
  const result = await runAction("addUserToGroup", "okta", authParams, {
    groupId: testGroupId,
    userId: testUserId,
  });
  ...
  console.log(`Successfully added user ${testUserId} to group ${testGroupId}.`);

  console.log("Okta addUserToGroup test completed successfully.");
}

runTest().catch((error) => {
  console.error("Okta testAddUserToGroup failed:", error.message);
